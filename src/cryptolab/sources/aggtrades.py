...
def _request_aggtrades(
    ...
    """
    Execute a Binance aggTrades REST request
    with retry logic.
    """
...
def _empty_raw_aggtrades() -> pd.DataFrame:
    """
    Return an empty raw aggTrades frame.

    Runtime point-in-time metadata is not attached
    because there is no observation row to timestamp.
    """
...
def _parse_aggtrades(
    ...
    """
    Parse Binance aggTrades JSON into the
    ...
    -----------------------
    available_at
        Derived from Binance trade_time.

    available_at_quality
        derived

    ingested_at
        Actual CryptoLab parse/receipt time
        for this REST response.
...
def fetch_aggtrades_from_id(
    ...
    """
    Fetch aggregate trades starting from
    agg_trade_id.

    This is the preferred primitive for
    deterministic forward pagination after
...
def fetch_aggtrades_by_time(
    ...
    This function is best used to locate a
    starting aggTrade ID. Once an ID is known,
    forward pagination should normally use
